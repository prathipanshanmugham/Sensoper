...
- [x] **TNEB Auto-Fetch + Solar Project Report + PDF Merge** — new module at `/dashboard/solar-report` and TNEB CTA on Site Visit (Customer step). Endpoints: `POST /api/tneb/fetch` (pluggable bill-fetch provider via `BILL_FETCH_PROVIDER`/`BILL_FETCH_API_KEY` env; manual-entry fallback when unset), `GET /api/solar/irradiation` (live NASA POWER, free), `POST /api/solar/sizing` (kWp + panels + inverter + battery + 25-yr financials + PM Surya Ghar subsidy + ROI + payback + CUF + CO₂ offset), `POST /api/solar/merge-pdf` (pypdf, prepend or append, 50MB cap). Frontend builds branded jsPDF report (Roboto for ₹), backend merges with user-uploaded PDF. *(May 2026)*
