...
- [x] **TNEB Auto-Fetch + Solar Project Report + PDF Merge** — Embedded inside the New Project wizard (Site & Electrical step) as `SolarReportSection`. NOT a standalone tab. Endpoints: `POST /api/tneb/fetch` (pluggable bill-fetch provider via `BILL_FETCH_PROVIDER`/`BILL_FETCH_API_KEY` env; manual-entry fallback when unset), `GET /api/solar/irradiation` (live NASA POWER, free), `POST /api/solar/sizing` (kWp + panels + inverter + battery + 25-yr financials + PM Surya Ghar subsidy + ROI + payback + CUF + CO₂ offset), `POST /api/solar/merge-pdf` (pypdf, prepend or append, 50MB cap — kept for future use). `solar_report` persisted on Project model. Quotation PDF in ProjectDetails now appends the full Solar Project Report section (consumer, recommended system, financial projection, technical KPIs, 25-yr breakdown) — **single download includes both the QR/quotation pages AND the solar report**. *(May 2026 — refactored to embedded May 2026)*
