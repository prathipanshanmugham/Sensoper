...
- [x] **Focus-loss bug fix + Reference Site feature** — AutoField/DriverField extracted to module scope to stop remount-per-keystroke (verified: typing "3500" into the EB bill input keeps all 4 chars). New first-class field **`reference_project_id`** on Project + new endpoints `GET /api/projects/reference-candidates?q=` (completed projects with thumbnail + metrics) and `GET /api/projects/{id}/reference-summary` (full reference detail). Searchable inline dropdown on the final wizard step with thumbnails, customer name, system size, location. Generated quotation PDF now auto-includes **"Project Metrics Summary"** (8-tile grid + Monthly-vs-Annual bar + payback timeline) and **"Reference Project Performance"** (header card with image + actual metrics + side-by-side comparison table) whenever the data is available. *(Feb 2026)*
