...
- Cosmetic: duplicate React key warning on CEO Dashboard; `PartnerDetail`/`ProjectPartnerCard` assign-dialog logic duplicated (~40 lines) — could extract a shared hook.
- Leftover `TEST_UI_*` / `TEST_*` seed rows in `partners`/`ecommerce_platforms`/`ecommerce_orders` collections from this test run — harmless, cleanup optional (same class as the pre-existing catalogue TEST-row housekeeping item).
