...
export default function DailyUpdatesPage() {
  ...
  const handleDelete = async (id) => {
    ...
    try { await dailyUpdatesAPI.delete(id); await fetchUpdates(); } catch (err) { console.error(err); }
  };
    ...
    <div className="py-6 px-4">
      <div className="max-w-5xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-sm text-slate-500">Log daily progress, materials, payments, and maintenance</p>
        </div>
          ...
          <>
            ...
            <Card className="border-slate-200 mb-6" data-testid="update-form">
              <CardHeader className="py-3 border-b border-slate-200">
                <CardTitle className="text-base font-['Outfit'] flex items-center gap-2"><Calendar className="h-4 w-4" />New {UPDATE_TYPES.find(u => u.id === activeType)?.label} Update</CardTitle>
              </CardHeader>
              <CardContent className="p-4 space-y-3">
                ...
                {activeType === 'payment' && (
                  <>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1"><Label className="text-xs">Amount Received (Rs) *</Label><Input type="number" value={paymentForm.amount} onChange={(e) => setPaymentForm(p => ({...p, amount: e.target.value}))} placeholder="0" data-testid="payment-amount" /></div>
              ...
              <CardContent className="p-4">
                {loading ? <div className="flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-emerald-600" /></div> :
                 ...
                 <div className="space-y-2">
                   {updates.map(u => {
                     const typeInfo = UPDATE_TYPES.find(t => t.id === u.update_type) || UPDATE_TYPES[0];
                     return (
                       <div key={u.id} className="flex items-start gap-3 p-3 border border-slate-200 rounded-lg hover:bg-slate-50" data-testid={`update-${u.id}`}>
                         ...
                         <Button variant="ghost" size="icon" className="h-8 w-8 text-red-400 shrink-0" onClick={() => handleDelete(u.id)} data-testid={`delete-update-${u.id}`}><Trash2 className="h-3.5 w-3.5" /></Button>
                       </div>
                     );
                   })}
                 </div>}
